...
def is_table_present(page):
    """
    Heuristic function to detect if a page contains a table based on the structure and patterns.
    """
    # Extract the page text
    text = page.get_text("text")
    
    # 1. Look for common table-like patterns in the text
    # Check for horizontal lines or columns often found in tables
    if "-----" in text or "=====" in text or "\n\n" in text:
        return True

    # Extract blocks from the page as a dictionary
    blocks = page.get_text("dict")["blocks"]
    
    # 2. Check alignment and block structure
    block_positions = []
    for block in blocks:
        if block["type"] == 0:  # Only consider text blocks (ignore images, etc.)
            block_bbox = block["bbox"]  # Get bounding box
            block_width = block_bbox[2] - block_bbox[0]  # Width
            block_height = block_bbox[3] - block_bbox[1]  # Height
            block_positions.append((block_bbox[0], block_bbox[1], block_width, block_height))
    
    if len(block_positions) < 2:  # A single block unlikely to be a table
        return False

    # Convert block positions to numpy array for analysis
    block_positions = np.array(block_positions)

    # 3. Check if multiple blocks share similar heights (indicating table rows)
    heights = block_positions[:, 3]
    avg_height = np.mean(heights)
    similar_heights = np.sum(np.abs(heights - avg_height) < avg_height * 0.2)  # Tolerance of 20%

    if similar_heights >= 3:  # If we have multiple blocks with similar heights, likely a table
        return True

    # 4. Check if blocks are aligned (vertical alignment indicating columns)
    x_positions = block_positions[:, 0]
    unique_x_positions = len(np.unique(np.round(x_positions, 2)))  # Unique x-coordinates with some rounding

    if unique_x_positions >= 3:  # If there are at least 3 aligned vertical columns, we likely have a table
        return True

    # 5. Detect column-like structures within the text
    lines = text.split("\n")
    column_pattern_found = False
    for line in lines:
        # Check if a line has multiple "words" separated by consistent spaces (indicating columns)
        if len(line.split()) > 3:  # More than 3 words in a line might indicate a table row
            column_pattern_found = True
            break

    if column_pattern_found:
        return True
...
def get_page_data(vector):
    ...
    log_messages = [f"Number of pages in the file: {num_pages} and metadata is {metadata}\n"]
    ...
    for page_no in tqdm(range(seg_from, seg_to), desc=f"Extracting {os.path.basename(filepath)} seg {idx}", disable=seg_to <= seg_from):
        ...
        # Extract OCR and page data
        start_time = time()
        ocr_page = page.get_textpage_ocr()
        log_messages.append(f"Time taken to execute get_textpage_ocr: {time() - start_time}\n")

        start_time = time()
        page_data = ocr_page.extractDICT()
        log_messages.append(f"Time taken to execute extractDICT: {time() - start_time}\n")

        # Extract words from page
        start_time = time()
        word_data_list = ocr_page.extractWORDS()
        log_messages.append(f"Time taken to execute extractWORDS: {time() - start_time}\n")

        # Conditionally run find_tables based on table presence
        start_time = time()
        if is_table_present(page):
            tables = page.find_tables()
            log_messages.append(f"Time taken to execute find_tables: {time() - start_time}\n")
        else:
            tables = []
            log_messages.append(f"Skipped find_tables for page {page_no}\n")
        ...
        # Insert table data if block is part of a table
        start_time = time()
        page_data, page_tables = block_table_data(page_data, tables, page_no, debug)
        log_messages.append(f"Time taken to execute block_table_data: {time() - start_time}\n")

        # Process word bounding boxes
        for word_data in word_data_list:
            ...
            bbox = [word_data[0] / page_data["width"], word_data[1] / page_data["height"], word_data[2] / page_data["width"], word_data[3] / page_data["height"]]
        ...
        log_messages.append(f"Time taken to execute clean_blocks: {time() - start_time}\n")

        # Group blocks and sort
        if page_data["blocks"]:
            all_groups = []
            ...
            # Sort groups by up-to-down and left-to-right
    ...
    with open("metrics_th_new.txt", "a") as log_file:
...
def pdf_parser(filepath, table_request=None, debug=False, debug_path="/tenant/logs/test_output"):
    metrics_file="metrics_th_new.txt"
