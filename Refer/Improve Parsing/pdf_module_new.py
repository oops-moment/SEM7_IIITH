...
def is_table_present(page_data):
    """
    ...
    for block in blocks:
        if block.get("type") == 0 and "text" in block:  # Ensure 'text' key exists
            text = block["text"]
    ...
    block_positions = []
    for block in blocks:
        if block.get("type") == 0 and "bbox" in block:  # Ensure 'bbox' key exists
