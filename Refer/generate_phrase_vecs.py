...
from densephrases.utils.single_utils import (
    set_seed,
    to_list,
    to_numpy,
    backward_compat,
...
from densephrases.scripts.generate_paragraph_vecs import start_store_sparse_embeddings, end_store_sparse_embeddings,start_store_paragraph_embeddings_with_phrases,start_store_paragraph_embeddings_initial, end_store_paragraph_embeddings
...
def dump_phrases(args, model, tokenizer, filter_only=False):
    ...
    output_dump_file = os.path.join(
        args.output_dir,
        f"{output_path}/{args.predict_file}.hdf5",
    )

    for predict_file in predict_files:
        args.predict_file = predict_file
        logger.info(f"***** Pre-processing contexts from {args.predict_file} *****")
        dataset, examples, features = load_and_cache_examples(args, tokenizer, evaluate=True, output_examples=True, context_only=True)
        sparse_start_time = timeit.default_timer()
        encoding_task, batch_lens = start_store_sparse_embeddings(features, args.predict_file, settings.dumps_dir)
        args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
        expansive_task = start_store_paragraph_embeddings_initial(features, args.predict_file, settings.dumps_dir, recreate=True)
        ...
        def get_phrase_results():
            for batch in tqdm(eval_dataloader, desc="Dumping"):
                ...
                for i, feature_index in enumerate(feature_indices):
                    ...
                    output = [to_numpy(output[i]) if type(output) != dict else {k: to_numpy(v[i]) for k, v in output.items()} for output in outputs]
        ...
        expansive_start_time = timeit.default_timer()
        # dependent upon the get_phrases function above.....
        start_store_paragraph_embeddings_with_phrases(features, args.predict_file, settings.dumps_dir, recreate=True)
        end_store_sparse_embeddings(args.predict_file, encoding_task, batch_lens, settings.dumps_dir)
        logger.info(f"timing: 1.5) {timeit.default_timer()-sparse_start_time:.3f}s: encode document sparse embeddings")
        end_store_paragraph_embeddings(args.predict_file, expansive_task, settings.dumps_dir, recreate=False)
        logger.info(f"timing: 1.5) {timeit.default_timer()-expansive_start_time:.3f}s: encode document expansive embeddings")
