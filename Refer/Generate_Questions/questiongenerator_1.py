...
class QuestionGenerator:
    ...
    def _get_MC_answers(self, correct_answer: Any, docs: Any) -> List[Mapping[str, Any]]:
        ...
        # Remove duplicate elements by converting to JSON and using a set
        entities_json = [json.dumps(kv) for kv in entities]
        pool = sorted(list(set(entities_json)))  # Convert to sorted list initially
        num_choices = min(4, len(pool)) - 1  # -1 because we already have the correct answer

        # Add the correct answer to final choices
        final_choices = []
        correct_label = correct_answer.label_
        final_choices.append({"answer": correct_answer.text, "correct": True})

        # Remove correct answer from the pool
        correct_answer_json = json.dumps({"text": correct_answer.text, "label_": correct_label})
        if correct_answer_json in pool:
            pool.remove(correct_answer_json)

        # Filter answers that match the correct label
        matches = [e for e in pool if correct_label in e]

        # Ensure matches and remaining_pool are sorted lists
        if len(matches) < num_choices:
            choices = matches
            remaining_pool = sorted([e for e in pool if e not in choices])  # Sorted list for remaining options
            choices.extend(random.sample(remaining_pool, num_choices - len(choices)))
        else:
            choices = random.sample(matches, num_choices)

        # Decode JSON strings back to dictionary format for choices
        choices = [json.loads(s) for s in choices]

        for choice in choices:
            final_choices.append({"answer": choice["text"], "correct": False})

        # Randomly shuffle final choices
